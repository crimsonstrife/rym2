    ...
    if ($action == 'edit') {
        ...
        if (!$hasUpdateRolePermission) {
            ...
            if (empty($role_id) || $role_id == null) {
                ...
                //include the error role file
                include_once(__DIR__ . '/../../includes/errors/errorMessage.inc.php');
            } else {
                //try to get the role information
                $object = $role->getRoleById(intval($role_id));
            ...
            if (!empty($object)) {
                ...
                foreach ($rolesArray as $roleArray) {
                    foreach ($roleArray as $roleObject) {
                        ...
                        $rolePermissions = $role->getRolePermissions(intval($role_id));
...
<div class="container-fluid px-4">
    <h1 class="mt-4"><?php echo $role->getRoleNameById(intval($role_id)); ?></h1>
    <div class="row">
        <div class="card mb-4">
            <!-- Edit Form -->
            <form
                action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?view=' . htmlspecialchars($_GET['view']) . '&role=' . htmlspecialchars($_GET['role']) . '&action=' . htmlspecialchars($_GET['action']) . '&id=' . htmlspecialchars($_GET['id']); ?>"
                            ...
                            <div class="form-group">
                                ...
                                <p>
                                    <input type="text" id="roleName" name="role_name" class="form-control"
                                        value="<?php echo $role->getRoleNameById(intval($role_id)); ?>"
                                        placeholder="<?php echo $role->getRoleNameById(intval($role_id)); ?>" required>
                                </p>
                            </div>
                            <div class="form-group">
                                <p>
...
<div class="container-fluid px-4">
    ...
    <div class="row">
        <div class="card mb-4">
            <!-- Create Form -->
            <form
                action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?view=' . htmlspecialchars($_GET['view']) . '&role=' . htmlspecialchars($_GET['role']) . '&action=' . htmlspecialchars($_GET['action']); ?>"
                method="post" enctype="multipart/form-data">
                <div class="card-header">
                    <div class="card-title">
                        <i class="fa-solid fa-scale-balanced"></i>
